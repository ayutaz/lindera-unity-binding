...
```
┌─────────────────┐     ┌──────────────────┐     ┌─────────────────┐
│  Unity (C#)     │────▶│  jslib           │────▶│  lindera-wasm   │
│  + UniTask      │     │  (JavaScript)    │     │  (WASM)         │
└─────────────────┘     └──────────────────┘     └─────────────────┘
```

### プラットフォーム抽象化
```
                    ILinderaTokenizer (インターフェース)
                              │
              ┌───────────────┼───────────────┐
              │                               │
    LinderaTokenizer              LinderaTokenizerWebGL
    (ネイティブ)                        (WebGL/WASM)
              │                               │
    LinderaTokenizerFactory.Create() で自動選択
```
...
| `webgl-deploy.yml` | WebGLビルド＆GitHub Pagesデプロイ | push/PR (Assets変更時) |
